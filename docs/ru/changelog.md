...
- **`tessera check` диагностирует конфигурацию ГОСТ до первого реального
  входа.** Предупреждает про `mode = "pkcs11"` с разрешённым
  ГОСТ-алгоритмом (не поддерживается: `cryptoki` ≤ 0.7 не даёт варианта
  механизма для `CKM_GOSTR3410`/2012), про `gost_engine_path` без
  ГОСТ-OID в whitelist (мёртвая конфигурация — движок никогда не
  загрузится), а при полностью корректной конфигурации либо
  подтверждает успешную загрузку движка (`gost_engine_ok`), либо
  показывает причину отказа (`gost_engine_load_failed`) — тем же кодом,
  которым пользуется сама аутентификация. Разбор строк —
...
- **ГОСТ-контейнер открывается на устройстве, где `gost-engine` не был
  ...
  назван хешем, которого в процессе нет, MAC не вычисляется вовсе, и
  OpenSSL может положить `mac verify failure` поверх настоящей причины.
  Модуль читал только эту строку и объявлял происходящее неверным PIN:
  инженеру предлагали ввести PIN заново до исчерпания попыток, а движок
  так и не загружался. Теперь строки о ненайденном алгоритме разбираются
  раньше строки о MAC. Набор этих строк зависит от версии OpenSSL и от
  того, почему именно не нашёлся алгоритм (`unknown digest algorithm`,
  `key gen error`, `mac generation error`, `inner_evp_generic_fetch:
  unsupported`; строки `mac verify failure` в стеке может и не быть
  вовсе) — поэтому разбирается набор признаков, а не одна фиксированная
  форма. Настоящий неверный PIN ни одного из них не порождает.

  Признаки при этом рассматриваются, только пока движок не загружен. Как
  только он загружен, отказ MAC действительно означает неверный PIN, а
  записи о неудачном поиске алгоритма могут остаться в очереди ошибок
  OpenSSL и после того, как поиск успешно завершился запасным путём.
  ...
  действительно не читается, и повторный ввод этого не изменит. **Меняется
  и код возврата PAM:** раньше такой вход заканчивался `PAM_MAXTRIES` (11)
  после исчерпания попыток, теперь — `PAM_PERM_DENIED` (6) на первой же.
  Стеки, которые различают эти коды (`pam_faillock`, разборы журнала в
  `fail2ban`), увидят другое событие: не «исчерпан лимит попыток», а
  «отказано». Число попыток загрузки движка при этом ограничено одной на
  процесс — после первой попытки движок либо загружен, либо его неудача
  запомнена.

  Неудачная загрузка движка теперь пишется в журнал (`tessera.gost`,
  уровень warn) с вариантом ошибки и настроенным путём — на каждой
  попытке входа, а не однократно. Инженеру по-прежнему показывается
  ошибка контейнера: путь к движку — забота администратора, и из
  приглашения на ввод PIN он не виден. Запись о неудаче кэшируется на всё
  время жизни процесса: `sshd` форкается на каждое соединение и этого не
  замечает, а дисплейный менеджер живёт одним процессом весь сеанс — там
  сломанный движок останется сломанным до перезапуска службы, даже если
  файл починили. Это осознанный размен: повторная загрузка подозрительной
  библиотеки на каждой попытке трогала бы глобальную таблицу ENGINE в
  libcrypto по инициативе того, кто стоит у приглашения, и всё равно
  ничего бы не исправила.

- **Streebog-256/512 резолвились по неверным NID.** `gost_2012_256_md`/
  `gost_2012_512_md` были захардкожены на NID `1177`/`1178` — это NID
  шифра «Кузнечик», а не хешей Стрибог; собственный doc-comment модуля
  при этом уже утверждал резолюцию по имени, только код ей не
  соответствовал. Теперь оба хеша резолвятся через
  `MessageDigest::from_name` (`EVP_get_digestbyname`) по статическим
  именам libcrypto (`md_gost12_256`/`md_gost12_512`), без хардкода
  второго, отдельно поддерживаемого числа.

- **`gost-engine` мог не загружаться повторно в одном процессе — типично
  на `fly-dm`, не воспроизводимо через `pamtester`.** Astra
  авторегистрирует движок `gost` при первом обращении к libcrypto в
  процессе (`/etc/ssl/openssl.cnf`, `default_algorithms = ALL`). У
  самого `gost-engine` есть process-global guard, который выставляется
  при первой успешной инициализации и не сбрасывается его же
  деструктором (`gost_eng.c`, `check_gost_engine`) — если
  ambient-регистрация Astra трогала libcrypto раньше, чем `tessera`
  успевала загрузить движок сама, повторная инициализация в этом же
  процессе становилась невозможна в принципе:
  `ENGINE_ctrl_cmd_string`-последовательность (`SO_PATH=1, ID=1,
  LOAD=0`) проваливалась на шаге `LOAD`, и никакой вызов OpenSSL ENGINE
  API это уже не чинил. В `sudo`/`login`/`sshd` процесс живёт одну
  попытку входа, поэтому гонка не успевала себя проявить; в `fly-dm`,
  где один процесс обслуживает все попытки входа за время работы
  дисплея, движок либо загружался первым же входом на устройстве, либо
  оставался сломанным до перезапуска службы. Self-check теперь
  выполняет загрузку движка по самому факту заданного
  `gost_engine_path` (а не по наличию ГОСТ-OID в whitelist) — это
  делает `tessera` первым, кто вообще трогает libcrypto в процессе, и
  гонка больше не возникает. Отключение `default_algorithms = ALL` в
  `openssl.cnf` остаётся возможной рекомендацией (на один источник
  авторегистрации меньше), но перестало быть обязательным условием —
  разбор и диагностика через `tessera check` — в `troubleshooting.md`
  §10.
...
- **OCSP-режимы отвергаются валидатором конфига.**
  ...
  обещал то, чего нет. История последующей реализации сохранена приватным
  Spec9-решением `auth.ADR-HIST-ocsp-support`.
