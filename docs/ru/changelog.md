...
- **Роль на логине — это имя учётной записи входа; суффикс `<user>+<role>`
  и PAM-prompt удалены.** Инженер входит в ролевую учётную запись, названную
  по роли (`ssh serv@device`), и запрошенная роль равна `PAM_USER`. Других
  источников роли не осталось. Вместе с суффиксом ушло и переписывание
  `PAM_USER`: пока модуль подменял имя, прочитанное стеком, разница между
  «до» и «после» подмены была доступна другим модулям стека — тот же класс,
  что вскрыл CVE-2021-3560 в polkit. Теперь подменять нечего. Имя учётной
  записи, не удовлетворяющее формату `role_id`
  (`^[a-z][a-z0-9-]{0,15}$`), отвергает вход до обращения к носителю.
  Личность инженера не теряется — она в удостоверении и журнале выпусков, а
  не в имени учётной записи. Миграция: вход `ivanov+serv@device` заменяется
  на `serv@device` с удостоверением, чей `pam_cert_allowed_roles` содержит
  роль `serv` (см. запись про единственный список допуска ниже). Ролевые
  учётные записи должны быть заведены на устройствах до обновления
  (провижининг — Census) как обычные, а не системные: вход в учётную
  запись, чей uid вне диапазона обычных пользователей, продукт
  отвергает. Остальные пути входа в них
  (`authorized_keys`, `su`, `sudo -u`, парольный вход, PAM-стеки без
  модуля) закрывают провижининг и администратор — см. install.md §8.4.
  Руководство по установке
  (`docs/{ru,en}/install.md`) переработано на ролевую учётную запись: выдача
  тестового удостоверения ведётся на роль `serv`, и добавлен раздел о ролевом
  хранилище устройства — без заполненного хранилища вход не проходит.
- **Допуск к учётной записи описывает один список — `pam_cert_allowed_roles`;
  расширение `pam_cert_user_binding`, аргумент `issuer issue-leaf --user` и
  секция `[[user_mapping]]` удалены.** Раз имя учётной записи входа и есть
  роль, `user_binding` («в какие учётные записи пущен предъявитель») и
  `allowed_roles` («какие роли вправе активировать») проверялись против одной
  и той же строки и утверждали одно и то же; различие, которое они сохраняли,
  описывало нереализуемое состояние «пущен в `serv`, но не вправе быть
  `serv`». Секция `[[user_mapping]]` удалена по другой причине: она была
  утверждением устройства о допуске («пущу любого, чей CN совпал»), а не
  выпускающего, — параллельный путь, где рамки назначает ограничиваемая
  сторона. Удостоверения чужой корпоративной PKI не поддерживаются, и
  обслуживать этот откат нечего. Конфиг, всё ещё содержащий
  `[[user_mapping]]`, отвергается при старте с диагностикой об удалении, а не
  общей ошибкой «неизвестное поле». Миграция: уберите секцию из
  `config.toml`; в командах выпуска замените `--user <учётная запись>` на
  `--role <роль>` — это одно и то же имя. Ранее выпущенные штатным путём
  удостоверения продолжают работать: `pam_cert_user_binding` игнорируется как
  любое неизвестное non-critical расширение, а `allowed_roles` они уже несут
  (без него вход не работал и раньше). Перевыпуск нужен только там, где вход
  держался на `[[user_mapping]]`, — но такое удостоверение и до этого
  изменения не проходило проверку роли. OID
  `2.25.215438916728501023845629178354627` считается отозванным и не будет
  переиспользован под другую семантику.
- **`issuer issue-root` и `issuer issue-ca` требуют `--allow-role` и не
  принимают `--max-ttl 0`.** Список ролей в рамках делегирования — закрытый
  белый список, поэтому пустой список разрешал не «любую роль», а ни одной;
  нулевой потолок TTL требовал от дочернего звена нулевого срока действия.
  Прежние умолчания (`--allow-role` необязателен, `--max-ttl 0`) выпускали CA,
  через который аутентификация не проходила никогда, — при этом сертификат
  выглядел валидным и попадал в журнал выдачи, а расхождение обнаруживалось
  только на устройстве, с диагностикой, указывающей на цепочку доверия, а не на
  аргументы команды выпуска. `--max-ttl` теперь имеет ненулевые умолчания по
  тому, что рамка ограничивает: `31536000` (год, срок CA организации) у
  `issue-root` и `14400` (4 часа, срок листа смены) у `issue-ca`. Миграция:
  добавьте `--allow-role` с именами ролей вашего внедрения; документированные
  примеры выпуска уже задают оба аргумента явно.
- **Ключ `[roles].enforce` удалён; роль проверяется всегда.** Прежде это
  был миграционный тумблер (`"false"` | `"warn"` | `"require"`) с
  умолчанием `"false"` — «роли не проверяются». Умолчание оказалось
  несовместимо с собственным выпуском: удостоверения штатного issuer
  всегда несут рамки делегирования, а вход без выбранной роли отвергает
  любую цепочку с рамками, то есть парк на умолчаниях не пускал никого по
  своим же удостоверениям. Промежуточное `"warn"` (проверить, записать,
  пропустить) убрано вместе с остальным: положения «проверить и
  пропустить» в модели доверия быть не должно. Конфиг, содержащий ключ в
  любом значении, отвергается при старте с диагностикой об удалении —
  намеренно громко, а не тихим ужесточением. Миграция: удалить ключ и
  убедиться, что база ролей под `[roles].dir` заполнена, иначе входы
  отклоняются fail-closed (это поведение не менялось). Парк, чьи
  удостоверения выпущены без `pam_cert_allowed_roles`, теряет возможность
  входа — альтернатива только перевыпуск.
- **`[trust.revocation].mode` теперь обязателен.** Пропуск секции
  `[trust.revocation]` целиком или ключа `mode` внутри неё — ошибка
  валидации конфига на старте. Раньше отсутствие режима молча
  означало `mode = "none"` (отзыв не проверяется), и отозванное
...
- **`pkcs11_locking_mode` по умолчанию — `mutex`, а не `os`.** Раньше по
  умолчанию вызовы PKCS#11-модуля шли без сериализации, потому что модуль
  просят про `CKF_OS_LOCKING_OK`. Заявленная поддержка этого флага
  потокобезопасности не гарантирует: `rtpkcs11ecp` 2.14.1 при
  одновременном `C_Initialize` из двух нитей роняет процесс
  необработанным C++-исключением. Модуль — cdylib внутри `sshd`, `login`
  и `fly-dm`, поэтому падает процесс аутентификации. Теперь по умолчанию
  каждый вызов сериализуется процесс-глобальным мьютексом; цена — около
  20 нс на вызов, что несопоставимо со временем `C_Sign` на устройстве.
  Миграция: инсталляциям, которым нужна параллельность и чей поставщик
  модуля её подтверждает, добавить `pkcs11_locking_mode = "os"` явно;
  режим никуда не делся. Явно заданное значение и раньше, и теперь имеет
  приоритет.

- **Команда `issuer serve` удалена; открытый `issuer` — инструмент выпуска в
  командной строке.** Веб-кабинет и агент выпуска поставляются отдельно, и
  вместе с командой ушли её feature и зависимость `tiny_http`. Модуль
  подтверждения оператором остался как библиотечный API за фичей `native` —
  внешние фронтенды подписи продолжают им пользоваться. Релизные бинари
  собираются с `cli,pkcs11,vault,file`. Миграция: браузерный выпуск — через
  коммерческий инструментарий, выпуск из командной строки не изменился.
- **Кабинет выпуска вынесен из репозитория; реестр подписывается на стороне
  агента.** Для этого у агента появился выделенный ключ подписи реестра
  (P-256, проверяется при старте) и конечная точка `/sign-registry`.
  Спецификация кабинета уехала вслед за кодом.
- **Токен, не сообщивший `CKA_EXTRACTABLE`, больше не считается
  подтвердившим неизвлекаемость ключа.** Инвариант режима B проверялся
  чтением атрибута, но `cryptoki` молча выбрасывает из результата всё, что
  токен отказался отдать: записи с `CK_UNAVAILABLE_INFORMATION`
  пропускаются, а `CKR_ATTRIBUTE_SENSITIVE` и `CKR_ATTRIBUTE_TYPE_INVALID`
  ошибкой не считаются. Парсер начинал с «неизвлекаем» и переписывал
  значение только при наличии атрибута, поэтому молчание провайдера было
  неотличимо от подтверждения, и ключ принимался. Теперь состояний три:
  подтверждённое `FALSE`, подтверждённое `TRUE` и «не сообщено»; последнее
  отклоняется отдельной ошибкой `ExtractableAttributeUnavailable` с
  указанием причины отказа. Если уточняющий запрос сообщает, что атрибут
  читаем, выполняется одиночное перечитывание — провайдеры, ломающиеся на
  смешанных шаблонах, известны, а прямое чтение `FALSE` и есть требуемое
  доказательство. Миграция: развёртывания, где токен атрибут не отдаёт,
  до обновления входили молча, после обновления получают отказ; вход
  восстанавливается ключом `pkcs11_allow_unreported_extractable = true`.
  Обходные ключи независимы: `pkcs11_allow_extractable_keys` разрешает
  только подтверждённое `TRUE` и неотвеченный атрибут больше не покрывает,
  чтобы оператору, которому нужна совместимость с молчащим модулем, не
  приходилось заодно соглашаться на экспортируемые ключи. Измерено:
  на Рутокен ЭЦП 3.0 поведение не меняется — токен отдаёт `CKA_EXTRACTABLE`
  штатно. От остального парка (SoftHSM2, JaCarta-2 GOST, Рутокен Lite) того
  же ожидаем, но по этому атрибуту не проверяли: если ваш модуль его не
  отдаёт, вход после обновления отклонится до установки
  `pkcs11_allow_unreported_extractable = true`.
- **Публичный API `tessera_core`:** поле `FoundPrivateKey::extractable`
  меняет тип с `bool` на перечисление `ExtractableState` — булево не может
  выразить «токен отказался отвечать», не соврав. Модуль
  `token::pkcs11` экспортирует новые типы `ExtractableState`,
  `ExtractableProbe` и `ExtractableKeyPolicy`.

### Security

- **Фазы сессии и account management действуют только от имени, под которым
  прошёл допуск.** `PAM_USER` принадлежит приложению, а не модулю, и между
  `pam_sm_authenticate` и `pam_sm_open_session` его значение никем не
  закреплено. Раньше сессионная фаза перечитывала имя и отдавала его как есть
  в MAC-метку сессии и в привилегированные хуки, а при ошибке чтения
  подставляла CN удостоверения — то есть личность инженера вместо имени
  ролевой учётной записи. Теперь прочитанное имя сверяется с ролью,
  зафиксированной в контексте аутентификации (имя ролевой учётной записи и
  есть роль), и расхождение отклоняется fail-closed: `PAM_SESSION_ERR` в
  `open_session`, `PAM_PERM_DENIED` в `acct_mgmt`, пропуск хуков в
  `close_session` (выход из системы блокировать нельзя). Запасного имени нет
  ни при каком исходе.
...
### Added
- **Вход по сертификату на Windows.** Провайдер учётных данных на экране входа
  и служба движка под `LocalSystem`; ядро проверки переиспользовано без
  изменений — всё, что понадобилось в нём поправить, это платформенные
  ограждения, логика проверки не двигалась. Инженер предъявляет съёмный
  носитель, выбирает роль, вводит PIN и попадает на рабочий стол. **Роль пока
  не даёт прав:** она выбирается, сверяется с тем, что допускает
  удостоверение, и попадает в журнал — но выдача прав остаётся за срезом,
  вместе с самым дорогим нерешённым вопросом дизайна. Доверие локальным
  файлам получило настоящую реализацию под Windows: конфигурация и ролевое
  хранилище читаются через него, а POSIX-политика вне Unix отказывает целиком.

- Один открытый host вместо сборок под feature-флаги: платформенные
  enforcement-бэкенды поставляются подписанными плагинами и выбираются
  в runtime секцией `[mac] backend`. Подпись проверяется до загрузки
  `.so`; любой отказ — деградация в stub-бэкенд.

### Fixed

- **PKCS#11-библиотека инициализируется один раз на процесс, а не на
  каждую загрузку бэкенда.** `Pkcs11Backend::load` звал `C_Initialize`
  при каждом вызове, хотя это процесс-глобальное состояние провайдера.
  Повторный вызов в лучшем случае возвращал
  `CKR_CRYPTOKI_ALREADY_INITIALIZED`, а одновременный — на `rtpkcs11ecp`
  2.14.1 убивал процесс. Теперь контекст живёт в процесс-глобальном
  реестре по канонизированному пути модуля: сколько бы ни было загрузок,
  инициализация одна. Разные модули в одном процессе по-прежнему
  получают независимые контексты. Создание контекста сериализовано
  независимо от `pkcs11_locking_mode` — два бэкенда могут быть настроены
  по-разному, а дефект провайдера процесс-глобален; сам режим теперь
  закрепляется за модулем при первой загрузке, расхождение логируется
  как `pkcs11_locking_mode_conflict`. `C_Finalize` не вызывается вообще:
  это процесс-глобальная операция над общей загруженной библиотекой, и
  она деинициализировала бы провайдера у постороннего потребителя
  PKCS#11 в том же процессе (`pam_pkcs11`, `sshd` с `PKCS11Provider`),
  который не может об этом узнать. Цена — модуль остаётся загруженным до
  завершения процесса. Дефект не ловился три года, потому что весь
  PKCS#11-контур проверялся на SoftHSM2, который гонку переживает.
- **Выпущенный лист смены снова пригоден для аутентификации.**
  `issuer issue-leaf` не клал в удостоверение ни `keyUsage`
  (`digitalSignature`), ни `extendedKeyUsage` (`clientAuth`), а проверка
  листа в модуле требует оба. Любой выпуск штатным путём отвергался на
  первом же гейте с диагностикой `trust: key usage missing required bit`
  — доступ не выдавался (fail-closed), но и выдаться не мог. Теперь лист
  несёт `keyUsage` с `digitalSignature` (critical) и `extendedKeyUsage`
  с `clientAuth`; самопроверка после подписи отказывается отдать
  артефакт, в котором любого из них нет. Дефект пережил обе половины
  тестов, потому что выпуск сверялся со своими ожиданиями, а модуль — со
  своими фикстурами: добавлена перекрёстная проверка, прогоняющая
  выпущенный лист через `pre_validate_end_entity` — ту же функцию,
  которой пользуется модуль.
- **Исчерпание попыток PIN возвращает `PAM_MAXTRIES` (11), а не 8.**
  `FlowError::pam_code` для класса `MaxTries` / `PinLocked` /
  `MaxAttemptsExceeded` отдавал `8` под именем `PAM_MAXTRIES`, но в
  `<security/_pam_types.h>` `8` — это `PAM_CRED_INSUFFICIENT`. Приложение
  вместо «попытки исчерпаны» получало «не удаётся получить данные
  аутентификации»: `login` и `sshd` при `PAM_MAXTRIES` прекращают
  дальнейшие запросы, при `PAM_CRED_INSUFFICIENT` такого сигнала нет, а
  администратор по коду возврата искал проблему с носителем вместо
  заблокированного PIN. Доступ при этом не выдавался — fail-closed не
  нарушался. Значение в документации (`architecture.md`, таблица кодов
  возврата) исправлено там же.
- **`issuer` по умолчанию ставит `profile_version = 0`, а не `1`.** Три
  подкоманды выпуска (`issue-root`, `issue-ca`, `issue-leaf`) без явного
  `--profile-version` записывали в удостоверение версию формата `1`, тогда как
  Engine без ключа `[trust].max_supported_profile_version` принимает максимум
  `0`. Любое удостоверение штатной выдачи отвергалось на устройстве с ошибкой
  «profile_version 1 exceeds supported maximum 0» — fail-closed сработал как
  задумано, но на паре «выпуск ↔ проверка» это означало неработоспособную
  конфигурацию по умолчанию. Дефолт выдачи опущен до `0` (текущая версия
  формата); потолок Engine не менялся. Кому нужен прежний вывод — передавайте
  `--profile-version 1` явно и поднимайте `max_supported_profile_version` на
  устройствах.
- **`allowed_signature_algorithms` сверяется по алгоритму, а не по имени,
  которым его называет libcrypto.** Раньше запись конфига сравнивалась
  посимвольно с той строкой, которую OpenSSL печатает для OID сертификата, —
  а печатает он длинное имя, если знает OID, и точечный OID, если не знает.
  Знает он разное на разных сборках и меняет ответ прямо в процессе, как
  только загружен движок: тот же ГОСТ-сертификат читался то как
  `1.2.643.7.1.1.3.2`, то как `GOST R 34.10-2012 with GOST R 34.11-2012
  (256 bit)`. Проверка алгоритма зависела от того, что libcrypto успел
  зарегистрировать, — на ГОСТ-развёртывании это давало отказ входа, не
  воспроизводимый на соседнем устройстве. Теперь сертификат читается как
  точечный OID (всегда, из его собственных байтов), запись конфига
  распознаётся по таблице имён, и сравниваются распознанные алгоритмы. Одну
  подпись можно записать любым её именем: `"sha256WithRSAEncryption"` и
  `"1.2.840.113549.1.1.11"` равнозначны. Оба написания года в ГОСТ-именах
  (`-2012-` и `-12-`, второе — из нашей же документации) распознаются.
  Кого затронет: тех, у кого в списке стоит имя вне таблицы —
  `"sha1WithRSAEncryption"`, `"RSASSA-PSS"`, `"ED25519"` и подобные. Такое
  имя теперь не совпадёт ни с одним сертификатом, и удостоверения с этим
  алгоритмом перестанут проходить — деградация в отказ входа (fail-closed),
  без выдачи лишнего доступа. Лечится записью точечного OID алгоритма;
  таблица имён — в `configuration.md`, секция `[trust]`.
- **ГОСТ-контейнер открывается на устройстве, где `gost-engine` не был
  загружен заранее.** Расшифровать ключ ГОСТ-контейнера libcrypto своими
  силами не может, а движок, прописанный хостом в `openssl.cnf`, в процессе
  модуля не регистрируется: этот файл читается только при инициализации с
  загрузкой конфига, которую биндинг не запрашивает. Контейнер выглядел
  контейнером с неподдерживаемым алгоритмом ключа, и вход не проходил.
  Теперь неудачная попытка открыть контейнер догружает движок по
  настроенному пути и повторяется один раз. Повтор происходит только там,
  где `gost_engine_path` задан. Неверный PIN не повторяется никогда.

  Заодно исправлена классификация самой ошибки. Когда MAC контейнера
  назван хешем, которого в процессе нет, OpenSSL не может вычислить MAC
  вообще и кладёт `mac verify failure` поверх настоящей причины
  (`pkcs12_gen_mac:unknown digest algorithm`). Модуль читал только верхнюю
  строку и объявлял это неверным PIN: инженеру предлагали ввести PIN
  заново до исчерпания попыток, а движок так и не загружался. Теперь
  строки о ненайденном алгоритме разбираются раньше строки о MAC.
  Настоящий неверный PIN ни одну из них не порождает, так что лишних
  попыток это не стоит.

  Следствие, которое стоит знать: на ГОСТ-контейнере верный и неверный PIN
  до загрузки движка неотличимы — MAC не считается, PIN в это ещё не
  участвует. Поэтому на устройстве, где `gost_engine_path` не задан,
  ГОСТ-контейнер даёт «контейнер не читается», а не «неверный PIN»: он
  действительно не читается, и повторный ввод этого не изменит. Число
  загрузок движка при этом ограничено одной на процесс — после первой
  попытки движок либо загружен, либо его неудача запомнена.
- **Движок ГОСТ загружается только по заданному пути — во всех путях кода,
  а не только при открытии контейнера.** Раньше поиск движка по
  унаследованной `OPENSSL_ENGINES` оставался достижим ещё из четырёх мест:
  проверки цепочки, проверки подписи CRL, challenge-response и проверки
  OCSP-ответа. Во всех четырёх признаком «нужен ГОСТ» служит содержимое
  предъявленного удостоверения, то есть данные, которые контролирует
  предъявитель: достаточно было принести самоподписанный сертификат с
  ГОСТ-OID подписи, чтобы процесс аутентификации нашёл по
  `OPENSSL_ENGINES` произвольную библиотеку, загрузил её и сделал
  реализацией по умолчанию для RSA, DSA, DH, RAND и всех хешей — на
  устройстве, которое ни на какой движок не соглашалось. Теперь загрузчик
  отвергает вызов без пути, и поиск по идентификатору удалён из кода
  целиком. Устройства с заданным `gost_engine_path` работают как прежде;
  на устройствах без него ГОСТ-удостоверение, ГОСТ-CRL и ГОСТ-ответ OCSP
  честно отвергаются (fail-closed), а не проверяются наполовину. Валидация
  конфига и раньше требовала `gost_engine_path` везде, где allow-list
  разрешает ГОСТ-подписи, так что рабочие ГОСТ-развёртывания это не
  затрагивает.

### Breaking

- **OCSP-режимы отвергаются валидатором конфига.**
  `[revocation].mode = "ocsp"` / `"crl_then_ocsp"` и любые `ocsp_*`
  ключи теперь дают ошибку валидации на старте. Раньше ключи
  принимались, но OCSP-проверка не выполнялась — конфиг молча
  обещал то, чего нет. OCSP не реализован; план — в
  `openspec/changes/ocsp-support`.
- **`crl_max_age_hours = 0` невалиден.** Допустимый диапазон —
  `1..=8760`. Ноль раньше означал «каждая CRL мгновенно stale» —
  бессмысленная конфигурация, которую принимали молча. Чтобы
  отключить bound по возрасту — опускайте ключ.
- **Extractable-ключ на PKCS#11-токене отклоняется по умолчанию.**
  Ключ с `CKA_EXTRACTABLE = TRUE` противоречит смыслу аппаратного
  токена (приватный ключ можно выгрузить). Opt-out для legacy-токенов:
  `pkcs11_allow_extractable_keys = true`.
- **Пустой `[trust].anchors` — ошибка валидации.** Раньше пустой
  список принимался, и любая валидация цепочки гарантированно падала
  на первой auth-сессии вместо старта демона.
- **Malformed `pam_cert_user_binding` — отказ аутентификации.**
  Расширение, которое есть в сертификате, но не парсится, раньше
  приводило к fallback на `[[user_mapping]]` — повреждённое (или
  подделанное) binding-расширение тихо понижало источник
  авторизации. Теперь fail-closed.
- **`syslog_facility` / `journald_priority` deprecated.** Ключи
  игнорируются (WARN в лог при наличии в конфиге); удалите их из
  `config.toml` на следующем переоформлении.
- **Подпись CRL верифицируется fail-closed.** Подпись каждой
  применимой CRL проверяется ключом её issuer'а из verified-цепочки;
  CRL, подписанные не issuer'ом (или с невалидной подписью),
  перестанут приниматься — раньше revoked-список такой CRL
  применялся без проверки подписи.
