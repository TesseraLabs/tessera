...
- **PKCS#11-библиотека инициализируется один раз на процесс, а не на
  ...
  инициализация одна, а `C_Finalize` выполняется при уничтожении
  последнего владельца. Разные модули в одном процессе по-прежнему
  получают независимые контексты. Создание и финализация контекста
  сериализованы независимо от `pkcs11_locking_mode` — два бэкенда могут
  быть настроены по-разному, а дефект провайдера процесс-глобален.
  Дефект не ловился три года, потому что весь PKCS#11-контур
  проверялся на SoftHSM2, который гонку переживает.
