...
- **Роль на логине — это имя учётной записи входа; суффикс `<user>+<role>`
  ...
  на `serv@device` с удостоверением, чей `pam_cert_user_binding` содержит
  `serv`, а `pam_cert_allowed_roles` — роль `serv`; это две разные проверки,
  и удостоверение, пускающее в учётную запись, но не разрешающее одноимённую
  роль, вход отклоняет. Ролевые учётные записи должны быть заведены на
  устройствах до обновления (провижининг — Census), вход в них закрыт всеми
  путями, кроме cert-аутентификации Tessera. Руководство по установке
  (`docs/{ru,en}/install.md`) переработано на ролевую учётную запись: выдача
  тестового удостоверения ведётся на роль `serv`, лист несёт три расширения
  (добавилось `pam_cert_allowed_roles`), и добавлен раздел о ролевом
