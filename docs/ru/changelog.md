...
- **ГОСТ-контейнер открывается на устройстве, где `gost-engine` не был
  ...
  где `gost_engine_path` задан: без него загрузка искала бы движок по
  унаследованной `OPENSSL_ENGINES` и делала бы найденное реализацией по
  умолчанию для RSA, DSA, DH, RAND и всех хешей — на хосте, который ни на
  какой движок не соглашался. Неверный PIN не повторяется никогда.
