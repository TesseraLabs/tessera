...
Это единственный процесс, в котором потеря носителя не означает утечки ключа:
скопировать удостоверение вместе с ключом нельзя, потому что закрытый ключ
физически не извлекается. Контейнера и пароля к нему в этом процессе нет вовсе —
только PIN токена.
...
Из запроса берутся только открытый ключ и субъект; скоуп задаёт выпускающий.
...
- Потеря или кража носителя не означает утечки ключа — нужен ещё и PIN, а
  неверный PIN блокирует устройство аппаратным счётчиком.
