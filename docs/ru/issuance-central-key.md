...
известен двум сторонам и пересекает границу между ними: само удостоверение уже
не указывает на инженера однозначно, и авторство приходится подтверждать
процедурой передачи — журналом выдачи и подтверждением получения.
...
sequenceDiagram
    ...
    O->>O: выпуск удостоверения (рамки задаёт он же)
...
Пароль контейнера по умолчанию порождает сам инструмент: 20 символов группами по
пять, из алфавита без похожих начертаний — его переписывают рукой. Показывается
он **один раз**, повторно не восстанавливается.

Свой пароль задаётся флагом источника: `--p12-passphrase-prompt` спросит
интерактивно, `--p12-passphrase-file` и `--p12-passphrase-stdin` возьмут из файла
или потока (см. [issuer.md](issuer.md#источники-секретов)). Минимальная длина —
12 символов. Значением аргумента командной строки пароль не передаётся, потому
что аргументы видны в списке процессов.
...
выпускающему, и авторство действий инженера подтверждается процедурой передачи,
а не самим удостоверением.
