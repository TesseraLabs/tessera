...
обращении к libcrypto в процессе (`/etc/ssl/openssl.cnf`,
`[engine_section]` → `default_algorithms = ALL`). У самого `gost-engine`
есть process-global guard (`ameth_GostR3410_2001` в его `gost_eng.c`),
который выставляется при первой успешной инициализации и никогда не
сбрасывается его собственным деструктором. Если ambient-регистрация
успела тронуть libcrypto раньше, чем `tessera` пытается загрузить движок
сама, движок в этом процессе уже проинициализирован один раз — повторная
инициализация тем же процессом больше невозможна в принципе, никаким
вызовом OpenSSL ENGINE API это не обойти.

**Что уже исправлено.** `self_check` выполняет загрузку движка по самому
факту заданного `gost_engine_path` (а не по наличию ГОСТ-OID в whitelist),
поэтому собственная загрузка `tessera` — первое, что вообще трогает
libcrypto в процессе: ambient-регистрация Astra уже не успевает сработать
раньше. Если ошибка всё равно воспроизводится на актуальной версии — это
сигнал, что что-то ещё в том же PAM-стеке трогает libcrypto раньше входа в
`pam_tessera` (нетипичный стек, сторонний PAM-модуль и т. п.), и разбирать
нужно именно это, а не считать проблему движка.

**Рекомендация (не обязательна, но убирает один движущийся узел):**
отключить `default_algorithms = ALL` в `[gost_section]`
`/etc/ssl/openssl.cnf` — тогда ambient-регистрация не происходит вовсе, и
единственная попытка инициализации `gost-engine` в процессе — та, которую
делает сама `tessera`.
...
| `gost_pkcs11_unsupported`        | WARN    | `mode = "pkcs11"` с разрешённым ГОСТ-алгоритмом: ГОСТ через PKCS#11-токен не поддерживается (`cryptoki` ≤ 0.7 не даёт варианта механизма для `CKM_GOSTR3410`/2012). |
| `gost_engine_configured_unused`  | WARN    | `gost_engine_path` задан, но в `allowed_signature_algorithms` нет ГОСТ-OID — движок никогда не будет загружен. Мёртвая конфигурация, скорее всего забытая после правки whitelist. |
| `gost_engine_load_failed`        | ERROR   | Конфигурация корректна, но реальная загрузка движка провалилась — см. разбор гонки выше. Аутентификация ГОСТ-сертификатов будет отказывать fail-closed, пока не исправлено. |
