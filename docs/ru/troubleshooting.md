...
`HostExtensionMissing`. На banner'е (TTY/sshd/sudo):
...
В журнале (ident `pam_tessera`) искать строку `tessera.auth:
...
PAM выводит на экран `PAM_TEXT_INFO` с диагностикой для
...
### `usb medium not found` после ожидания
...
[configuration.md](configuration.md#глобальные-параметры)).

### PKCS#11-токен не виден (`pcscd` не запущен)
...
**Симптом:** на USB Ventoy / с несколькими разделами между
`trying USB candidate` и завершением модуля проходит 10–30 с.
Длительность = число разделов × таймаут монтирования.

**Диагностика:** на 0.4.0 каждый кандидат логируется пошагово
(INFO `tessera.flow`):
...
Если per-candidate строк в логах нет вовсе — сборка старше 0.3.6
(там добавлено пошаговое логирование); обновитесь.
...
**Так задумано.** `tessera` — жёсткий второй фактор: без валидного
...
### `is not root-controlled` — демон или `tessera check` не стартуют

**Симптом.** `tessera daemon` или `sudo tessera check` завершаются с
сообщением вида `config path /etc/tessera/config.toml is not root-controlled`
либо `gost_engine_path /opt/gost/gost.so is not root-controlled`, с указанием
конкретной причины: владелец, режим доступа или символическая ссылка.

**Причина.** Демон и `tessera check` читают конфигурацию по той же политике,
что и PAM-модуль: файл конфигурации и все названные в нём пути должны
принадлежать root и быть недоступны на запись остальным. Раньше эту проверку
проходил только PAM, а демон и префлайт довольствовались обычным чтением.

Так сделано потому, что оба процесса загружают названные в конфигурации
нативные модули в собственное адресное пространство, а gost-engine к тому же
становится реализацией по умолчанию для целого набора алгоритмов. Путь,
который может переписать посторонний пользователь, — это исполняемый код в
привилегированном долгоживущем процессе.

**Что именно проверяется:** сам `config.toml`, доверенные якоря и
промежуточные удостоверения, файлы CRL, `gost_engine_path`, а также
`pkcs11_module` — во всех конфигурациях, где модуль действительно
загружается: при `mode = "pkcs11"` и при `mode = "pkcs12"` с носителем
`pkcs12_source = "token_object"`.

**Что делать.**

```bash
# Найти путь, на который ругается сообщение, и посмотреть его права.
sudo ls -la /etc/tessera/config.toml
sudo namei -l /opt/gost/gost.so   # покажет права на каждом звене пути

# Привести к ожидаемому виду: владелец root, без записи для группы и прочих.
sudo chown root:root /etc/tessera/config.toml
sudo chmod 0644 /etc/tessera/config.toml
```

Проверка смотрит на весь путь целиком, поэтому каталог, доступный на запись
постороннему, отвергнет и правильный по правам файл внутри него. Ослаблять
проверку нечем и не нужно — она отделяет исправную установку от той, где
посторонний может подменить исполняемый код.

### monitord недоступен или не запускается

**Симптомы (один кейс, две грани):**

- PAM отказывает `monitord unavailable` или зависает — демон
  формально жив, но IPC-сокет недоступен;
- `systemctl status tessera` показывает `failed` — демон не
  поднимается вовсе.
...
- сокет `/run/tessera/monitord.sock` не создан или занят → проверить
  `RuntimeDirectory=tessera` в юните и `lsof` на сокете;
- права на `/run/tessera/` неверны → должно быть
  `drwxr-x--- tessera tessera` (0750). Каталог, владельца
  (`User=tessera` / `Group=tessera`) и режим
  (`RuntimeDirectoryMode=0750`) создаёт сам юнит при старте — чинится
  через `sudo systemctl restart tessera`, а не ручным `chown`/`chmod`;
- `config.toml` повреждён → прогнать валидацию без старта демона:
  `sudo /usr/bin/tessera check` (или запустить демон в форграунде
  `sudo /usr/bin/tessera daemon --config /etc/tessera/config.toml` и
  читать диагностический вывод);
- отсутствует `gost-engine` → `openssl engine gost -t` без пометки
  `[ available ]`.
...
**Симптом:** извлечение USB корректно детектится в journald
(`grace window expired, dispatching action`), но logout не
происходит:
...
**Причина 1 (типичная):** строка `session ... pam_tessera.so` стоит
выше `@include common-session` (где `pam_systemd.so`), поэтому
`sm_open_session` срабатывает раньше `pam_systemd` и `XDG_SESSION_ID`
ещё не создан. `integrate-pam.sh` ставит session-строку ПОСЛЕ
`@include common-session`; демон валит старт с
...
**Причина 1 (наиболее частая):** `@include tessera-only` оказался
ПЕРЕД `auth required pam_parsec_mac.so`. `tessera-only` использует
...
**Причина:** legacy-поле `[mac].enabled` из старых конфигов; оно
удалено и заменено на `[mac].runtime`.
...
(`libfly-dm_greet_modern.so`) подставляет в заголовок жёстко зашитую
строку «Усиленный уровень защищенности». GreetString и PAM-сообщения
игнорируются.
...
3. Обновить CRL на endpoints (см.
   [operations.md §2.2](operations.md#22-обновление-crl)). Юнит
   `tessera-crl-update.service` пакетом не поставляется — его создаёт
   оператор по operations.md §2.2; где он настроен, обновление можно
   ускорить разовым `systemctl start tessera-crl-update.service`.
4. Проверить журнал:
   ```bash
...
**Симптом:** `sudo tessera check` выводит `[ERROR] gost_engine_load_failed:
... ENGINE_ctrl_cmd_string sequence (SO_PATH=1, ID=1, LOAD=0) failed for
<путь>`, при этом разовый `openssl engine gost -t` в отдельном процессе
показывает `[ available ]` без проблем. Чаще всего это долгоживущий
процесс — типично `fly-dm`, а не `sudo`/`pamtester` (каждый из них —
отдельный процесс на попытку входа).

**Причина.** Astra по умолчанию авторегистрирует движок `gost` при первом
обращении к libcrypto в процессе: `/etc/ssl/openssl.cnf` связывает
`[engine_section]` с секцией движка строкой `gost = gost_section`, а сам
ключ `default_algorithms = ALL` лежит в `[gost_section]`.

У самого `gost-engine` есть process-global guard — статическая переменная
`ameth_GostR3410_2001` в `gost_eng.c`. Функция `check_gost_engine()`
проверяет её и при непустом значении печатает `GOST engine already loaded`
и отказывает в повторной инициализации. Сбросить guard мог бы деструктор
движка (`gost_engine_destroy` обнуляет её вместе с остальными методами
`gost_meth_array`), но пока движок зарегистрирован в libcrypto, деструктор
не вызывается. Поэтому на практике: если ambient-регистрация успела
тронуть libcrypto раньше, чем `tessera` пытается загрузить движок сама,
повторная инициализация в этом процессе не пройдёт, и обойти это вызовами
OpenSSL ENGINE API нельзя.

Проверено по исходникам `gost-engine` v3.0.3 — последнего выпуска на
момент записи. Если в вашем дистрибутиве версия другая, сверяйте
`gost_eng.c` по ней.

**Что изменено.** `self_check` выполняет загрузку движка по самому факту
заданного `gost_engine_path`, а не по наличию ГОСТ-OID в whitelist. Это
переносит загрузку в самую раннюю точку процесса, которую `tessera`
контролирует, — до разбора контейнера и до обращения к вендорскому
PKCS#11-модулю.

Первым, кто трогает libcrypto в процессе, `tessera` при этом не становится
и стать не может: `pam_tessera` — библиотека внутри чужого процесса, а
`sshd` работает с libcrypto ещё на согласовании ключей, задолго до
PAM-стека. Ранняя загрузка сужает окно, в которое успевает вклиниться
ambient-регистрация, но не закрывает его. Если ошибка воспроизводится,
проверять нужно, что именно тронуло libcrypto раньше входа в
`pam_tessera`, — это может быть и сторонний PAM-модуль, и сам процесс
службы.

**Рекомендация (не обязательна, но убирает один движущийся узел):**
отключить `default_algorithms = ALL` в секции `[gost_section]`
`/etc/ssl/openssl.cnf` — той, на которую ссылается `[engine_section]`
строкой `gost = gost_section`. Тогда ambient-регистрация не происходит
вовсе, и единственная попытка инициализации `gost-engine` в процессе — та,
которую делает сама `tessera`.

### Диагностика `tessera check`

`sudo tessera check` — основной инструмент проверки ГОСТ-конфигурации; в
отличие от разового `openssl engine gost -t`, он выполняет ту же загрузку
движка, что и реальная аутентификация. Четыре возможные строки:

| Строка                          | Уровень | Значение                                                                 |
|----------------------------------|---------|---------------------------------------------------------------------------|
| `gost_engine_ok`                 | INFO    | `gost_engine_path` задан, ГОСТ-алгоритм разрешён whitelist'ом, движок успешно загрузился. |
| `pkcs11_openssl_unsupported`     | ERROR   | `mode = "pkcs11"` вместе с `crypto_backend = "openssl"`: в этой комбинации отвергается любое удостоверение, не только ГОСТ. Отдельно про ГОСТ: механизмы `CKM_GOSTR3410`/2012 стандартные (0x1201 и далее), в `cryptoki` для них нет вариантов, а лазейка `VendorDefined` принимает только значения от `CKM_VENDOR_DEFINED` (0x8000_0000) и выше. |
| `gost_engine_configured_unused`  | WARN    | `gost_engine_path` задан, в `allowed_signature_algorithms` нет ГОСТ-OID, движок при этом загрузился. Входу ничто не мешает — движок нужен и без ГОСТ-OID, — но настройка выглядит забытой после правки whitelist. Если движок в такой конфигурации не загрузится, запись будет `gost_engine_load_failed`. |
| `gost_engine_load_failed`        | ERROR   | `gost_engine_path` задан, но движок не готов — см. разбор гонки выше. Отказывает **любая** аутентификация, а не только по ГОСТ-удостоверениям: самопроверка обрывается на движке раньше, чем доходит до разбора удостоверения, поэтому вход по RSA и ECDSA на такой машине тоже не проходит. Пока путь не убран или движок не починен, войти нельзя никому. |

Если `gost_engine_path` не задан и ГОСТ не разрешён whitelist'ом —
`tessera check` не выводит по ГОСТ ничего: узел просто не использует
ГОСТ.
