...
Секреты бэкендов — PIN токена для PKCS#11 и пароль ключа для файлового
бэкенда — запрашиваются по одной лестнице источников:
...
1. **Источник, заданный флагом.** `--pinentry <path>` — внешний
   ...
   ключа файлового бэкенда. Заданный источник используется без обращения к
   остальным. Два явных источника одновременно — ошибка разбора аргументов.
