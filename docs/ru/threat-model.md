...
агент `issuer serve` (мост браузер ↔ бэкенд подписи). Инструмент не кастодиан:
...
| 11.1.3 | Статика веб-кабинета (SPA + WASM)    | подмена артефакта при доставке                                                             | кабинет встроен в подписанный бинарь `issuer` и раздаётся агентом same-origin по умолчанию — целостность за подписью бинаря, без CDN-посредника; для внешней раздачи (`--cabinet-dir`/статика) остаётся манифест `SHA256SUMS`; ключи недостижимы даже из подменённого SPA (подпись за агентом + токеном). Остаточный риск — см. 11.3 |
