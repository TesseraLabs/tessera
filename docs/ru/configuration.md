...
| `gost_engine_path`         | путь               | `None`      | абсолютный путь к `.so`                                        | Явный путь к `gost-engine`; обязателен, если OpenSSL разрешает ГОСТ-подписи (жизненный цикл движка — ниже). | Неявный поиск через `OPENSSL_ENGINES` отклоняется; файл и все предки должны контролироваться root. |
...
| `monitor_fail_mode`        | строка             | `"strict"`  | `"strict"`, `"permissive"`                                     | Пробрасывать ли нефатальные ошибки IPC с `monitord` вызывающему коду (`strict`) или глотать с WARN (`permissive`). | `DeviceGone`/`Unauthorized` фатальны всегда. В strict-режиме потеря наблюдения за токеном приравнивается к извлечению носителя. |
...
#### Жизненный цикл `gost-engine` в процессе

`gost-engine` тоже инициализируется не более одного раза за процесс, но
жёстче, чем PKCS#11-модуль: аналога повторного `C_Initialize` у него нет.
Внутри самого движка есть process-global guard (`ameth_GostR3410_2001` в
его `gost_eng.c`), который выставляется при первой успешной инициализации
и не сбрасывается его собственным деструктором. Если что-то в процессе —
например, Astra-авторегистрация через `default_algorithms = ALL` в
`/etc/ssl/openssl.cnf` — успевает тронуть libcrypto и тем самым
инициализировать `gost-engine` раньше `tessera`, повторная инициализация
в этом же процессе становится невозможна в принципе:
`ENGINE_ctrl_cmd_string`-последовательность (`SO_PATH=1, ID=1, LOAD=0`)
проваливается на шаге `LOAD`, и восстановиться в рамках того же процесса
уже нечем.

Поэтому загрузка движка встроена в самый ранний self-check и выполняется,
как только задан `gost_engine_path`. Это самая ранняя точка процесса,
которую контролирует `tessera`, — но не первое обращение к libcrypto в
процессе: модуль работает внутри чужого процесса, а тот же `sshd`
использует libcrypto на согласовании ключей задолго до PAM-стека. Окно, в
которое успевает вклиниться ambient-регистрация, сужается, но не
закрывается. В долгоживущих процессах вроде `fly-dm` (один slave на всё
время работы дисплея — см. пример выше про PKCS#11) именно это окно
решает, будет ли работать ГОСТ до перезапуска службы.

Загрузка привязана к заданному пути, а не к whitelist'у, ещё и потому, что
движок нужен там, где ГОСТ-OID не разрешён вовсе: для ГОСТ-ключа в
сертификате, подписанном RSA, и для контейнера под ГОСТ-PBE.

Отключение `default_algorithms = ALL` в `/etc/ssl/openssl.cnf` не
обязательно, но убирает лишний движущийся узел, если хочется на один
источник авторегистрации меньше —
разбор симптома и рекомендация: [troubleshooting.md
§10](troubleshooting.md#10-установка--gost-engine).
...
| `token_poll_interval_seconds` | целое | `2`     | `1..=600`            | Как часто демон опрашивает присутствие PKCS#11-токена. Применяется, когда удостоверение лежит на токене (`pkcs12_source = "token_object"` либо `mode = "pkcs11"`); для раздела съёмного носителя не используется — там работают события udev. | Задаёт задержку обнаружения извлечения. Больший интервал — дольше живёт сессия после того, как носитель унесли. |

**Обнаружение извлечения токена.** У съёмного носителя пропажа приходит
событием, у токена — опросом, поэтому появляется задержка. Пропажу должны
подтвердить **три опроса подряд**: одиночный сбой службы смарт-карт не гасит
сессию инженера, у которого токен на месте. При исправном опросе носитель
признаётся пропавшим не позже, чем через `3 × token_poll_interval_seconds` плюс
`usb_removed_grace_seconds` — около 6 секунд при значениях по умолчанию и
нулевой выдержке. Это срок **обнаружения**; выполнение действия (блокировка
сессии через `logind`) занимает своё время сверх него. Зависший вызов к
устройству ждётся не дольше 5 секунд и засчитывается как неудачный опрос.

**Потеря наблюдения.** Опрос может перестать отвечать: пропал считыватель,
зависла библиотека производителя. Три неудачных опроса подряд считаются потерей
наблюдения, и её смысл задаёт `monitor_fail_mode`: `strict` обещает непрерывное
присутствие носителя и поэтому приравнивает потерю наблюдения к извлечению;
`permissive` такого обещания не даёт и только пишет ошибку в журнал. Зависший
вызов к устройству засчитывается как неудачный опрос, а не как присутствие.

**Перезапуск демона.** После перезапуска присутствие устанавливается первым же
опросом, а не ожиданием события, поэтому извлечение, случившееся за время
перерыва, не теряется. Максимальное окно без наблюдения складывается из времени,
за которое демон признаёт опрос зависшим (два цикла), `WatchdogSec` из юнита,
задержки перезапуска и первого опроса — **66 секунд при значениях по умолчанию**.
Каждая секунда, добавленная к `WatchdogSec`, расширяет это окно один к одному.
...
### 3.1 Терминал — оффлайн, CRL с TTL, ключ на токене

Свойства: машина в железной коробке, нет Интернета, ключ — на токене.
Присутствие токена наблюдается опросом, поэтому его извлечение завершает сессию
так же, как извлечение флешки, — но не мгновенно, а в пределах интервала опроса
(см. «Обнаружение извлечения токена» выше). Там, где секунды задержки
неприемлемы, ограниченный срок роли остаётся дополнительной мерой, а не
заменой.
