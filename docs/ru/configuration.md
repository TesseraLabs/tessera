...
| `pkcs11_locking_mode`      | строка             | `"mutex"`   | `"os"`, `"mutex"`                                              | Стратегия блокировок PKCS#11.                                 | `"mutex"` — каждый вызов модуля сериализуется процесс-глобальным мьютексом; цена ≈ 20 нс на вызов. `"os"` снимает сериализацию и допустим только если поставщик модуля подтверждает потокобезопасность: заявленная поддержка `CKF_OS_LOCKING_OK` таковой не является. |
