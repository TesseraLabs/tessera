...
| `suspend_grace_seconds`      | целое  | —       | секунды              | Окно после resume, в котором события извлечения игнорируются (дефолт 30). | Слишком большое окно ослабляет реакцию на извлечение.      |
