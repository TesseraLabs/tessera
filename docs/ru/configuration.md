...
как только задан `gost_engine_path`, — это делает `tessera` первым, кто
вообще трогает libcrypto в процессе, и опережает ambient-регистрацию. В
долгоживущих процессах вроде `fly-dm` (один slave на всё время работы
дисплея — см. пример выше про PKCS#11) это единственный надёжный способ;
поведение при этом одинаково для всех режимов запуска.

Отключение `default_algorithms = ALL` в `/etc/ssl/openssl.cnf` не
обязательно (`tessera` сама выигрывает гонку), но убирает лишний
движущийся узел, если хочется на один источник авторегистрации меньше —
