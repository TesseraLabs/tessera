...
выпусков, а не в имени учётной записи. Ролевые учётные записи провижинятся
отдельно (Census); вход в них закрыт всеми путями, кроме
cert-аутентификации Tessera.

Допуск проверяется двумя расширениями удостоверения, применяемыми к одному и
тому же имени: `pam_cert_user_binding` («предъявителю разрешено входить в эту
ролевую учётную запись») и `pam_cert_allowed_roles` («предъявителю разрешено
активировать эту роль»). Удостоверение, пускающее в учётную запись `serv`, но
не разрешающее одноимённую роль, — законная конфигурация, и вход по нему
отклоняется.
