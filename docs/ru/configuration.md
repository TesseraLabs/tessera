...
| `source`  | путь   | `/var/lib/tessera/tags.toml` (standalone) / каталог role-store (managed) | абсолютный путь | Файл тегов либо каталог с манифестом.                  | Права на файл источника — часть границы доверия.            |
