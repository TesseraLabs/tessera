...
`bundle_version`, режим; событие `device_enrolled` уходит в audit. Поле serial
сейчас всегда пустое (`-`): при импорте контейнер ничем не заверен, а
определить без пароля, каким сертификатом устройство будет
аутентифицироваться, невозможно — поле останется пустым, пока серийник не
станет приходить из источника, покрытого подписью. Без тегов
