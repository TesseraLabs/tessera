...
См. [install.md §1–§9](install.md). Все секции выполняются
...
`bundle_version`, режим; событие `device_enrolled` уходит в audit. Серийник
сейчас не заполняется: при импорте контейнер ничем не заверен, а определить без
пароля, каким сертификатом устройство будет аутентифицироваться, невозможно —
поле останется пустым, пока серийник не станет приходить из источника,
покрытого подписью. В отчёте пустое значение печатается как `-`, а в событии
`device_enrolled` поле уходит пустой строкой (`serial=""`) — искать в журнале
надо именно её. Без тегов
