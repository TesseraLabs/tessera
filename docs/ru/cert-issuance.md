# Выпуск сертификатов: host_binding и allowed_roles

## Введение

Документ адресован CA-администратору: он объясняет, какие
X.509-расширения зашивать в leaf-сертификат Tessera, и приводит готовые
фрагменты `openssl.cnf`, по которым сертификат выпускается штатным
`openssl x509 -req`.

> Про то, **кто и где** порождает ключ и как удостоверение доезжает до
> устройства, — [issuance-workflows.md](issuance-workflows.md): пять процессов
> выпуска и выбор между ними. Здесь — про содержимое сертификата.

Расширений пять. Два — обязательные: они кодируют авторизацию «в какой
роли на каком хосте» и проверяются PAM-модулем на этапе
аутентификации:

- `pam_cert_host_binding` — на каких хостах сертификат действует;
- `pam_cert_allowed_roles` — какие роли сертификат вправе активировать.

Эти два расширения и только они определяют область действия
сертификата. Отдельного списка разрешённых учётных записей нет: имя
...
Отката на стороне устройства — механизма, которым устройство разрешало
бы вход по своим правилам, — не существует: рамки назначает
выпускающий, а не ограничиваемая сторона.

Остальные три — опциональные, под конкретные возможности; каждое
разобрано в своём разделе ниже:

- `pam_cert_max_integrity` — потолок метки целостности на МКЦ-хостах Astra;
- `pam_cert_profile_version` — версия формата сертификата (version-gate);
- `pam_cert_delegation_constraints` — рамки делегирования для
  промежуточного CA.

## OID-таблица

| Имя расширения | Дотированный OID | ASN.1 синтаксис | Критичность |
|---|---|---|---|
...
| `sha256:<HEX>` | разрешено только на хосте, чей `host_id_hash` совпадает с указанным 64-символьным lowercase-hex (case-insensitive) |
| Любая другая UTF-8 строка | строка интерпретируется как «сырое» `machine_id` и сравнение идёт через SHA-256 от строки |

В `pam_cert_allowed_roles` запись — это `role_id`, обязанный матчить
`^[a-z][a-z0-9-]{0,15}$`. Подстановка `*` здесь не поддерживается:
список ролей всегда перечисляется явно. Подробнее — раздел
[«Расширение `allowed_roles`»](#расширение-allowed_roles-выбор-роли-на-логине).

Для авторизации сертификата на конкретном хосте и в конкретной ролевой
учётной записи нужна **хотя бы одна совпавшая запись** в каждом из двух
расширений.

## Сценарий 1 — рабочая станция: один хост, одна роль

Рабочее место конкретного оператора. Сертификат можно использовать
только на машине с известным `machine_id` и только для входа в ролевую
учётную запись `oper`.
...
# Роль, она же учётная запись входа: единственная
...
Личность инженера (`CN`, `subjectAltName`) на допуск не влияет: она
нужна журналу выдачи и разбору инцидентов, а решение принимается по
двум расширениям выше.
...
## Сценарий 2 — оператор терминалов: несколько хостов, одна роль
...
## Сценарий 3 — мобильный администратор: любой хост, точная роль
...
allowed_roles по-прежнему остаётся жёсткое ограничение на роль, то есть
на имя учётной записи входа.
...
| Запись | Совпадает с… |
|---|---|
| `*` (host_binding) | любым хостом |
| `sha256:<HEX>` | хостом, чей `host_id_hash` равен `HEX` (без учёта регистра) |
| `<raw>` (host_binding) | хостом, чей `host_id_hash` равен `sha256(raw)` |
| `<role>` (allowed_roles) | входом в ролевую учётную запись с точным именем `<role>` |
| Расширение отсутствует | **отказ** (`HostExtensionMissing`; для allowed_roles — роль не покрыта) |
| Расширение пустое или DER-битое | **отказ** (`*ExtensionMalformed`) |
| Записи есть, но ни одна не совпала | **отказ** (`HostNotAllowed` / роль не покрыта) |
...
## Расширение `allowed_roles` (выбор роли на логине)
...
Запрошенная роль — это имя учётной записи входа (`ssh serv@device`).
Семантика авторизационная: роль покрыта, если её `role_id` присутствует
в списке.

Это единственный список допуска. На вопрос «в какую учётную запись пущен
предъявитель» он отвечает тем же перечислением, потому что имя учётной
записи входа и есть роль: удостоверение с `allowed_roles = [oper]` во вход
в `serv` не пускает. Отдельного списка учётных записей нет — два списка над
одной строкой описывали бы нереализуемое состояние «пущен в `serv`, но не
вправе быть `serv`».

OID: `2.25.185305973969816596290730578528098241367`

Структура (DER) — та же, что у host_binding:
...
сертификат не даёт ролей, то есть по нему нельзя войти: роль требуется всегда
и покрытие доказывается именно этим расширением.
...
## Расширение `profile_version` (version-gate)
...
Текущая версия формата — **`0`**. Других версий пока не существует: `issuer` без
явного `--profile-version` выпускает удостоверения именно с версией `0`, и это
единственная версия, которую Engine принимает без правки конфига. Значение выше
нуля имеет смысл выставлять только для парка, где
`[trust].max_supported_profile_version` уже поднят на всех устройствах, — иначе
выданное удостоверение будет отвергнуто на первой же аутентификации.

Фрагмент `openssl.cnf` для текущей версии (`0`):

```ini
2.25.107983357797077476746994938370032043240 = critical,ASN1:INTEGER:0
```

Эквивалент DER-строкой (`INTEGER 0`): `critical,DER:02:01:00`.

## Расширение `delegation_constraints` (рамки делегирования)
...
`pam_cert_allowed_roles = <role>` и стандартный
...
дополнительно `pam_cert_max_integrity` (см. §«Расширение `MAX_INTEGRITY`»).
