...
Расширений шесть. Два — обязательные: они кодируют авторизацию «какой
пользователь на каком хосте» и проверяются PAM-модулем на этапе
аутентификации:

- `pam_cert_host_binding` — на каких хостах сертификат действует;
- `pam_cert_user_binding` — для какого PAM-пользователя.
...
Остальные четыре — опциональные, под конкретные возможности; каждое
разобрано в своём разделе ниже:

- `pam_cert_max_integrity` — потолок метки целостности на МКЦ-хостах Astra;
- `pam_cert_allowed_roles` — роли, которые сертификат вправе активировать
  на логине;
...
Это не то же самое, что `pam_cert_user_binding`: тот отвечает на вопрос
«пущен ли предъявитель в эту учётную запись», а `allowed_roles` — «разрешено
ли ему активировать эту роль». В модели ролевых учётных записей обе проверки
применяются к одной строке, но остаются раздельными: удостоверение с
`user_binding = [serv]` и `allowed_roles = [oper]` во вход в `serv` не
пускает.
