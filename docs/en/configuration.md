...
| Field                      | Type      | Default  | Allowed values                                            | Effect                                                   | Security implication                                                  |
|----------------------------|-----------|----------|-----------------------------------------------------------|----------------------------------------------------------|------------------------------------------------------------------------|
| `mode`                     | string    | — (required) | `"none"`, `"crl"`, `"ocsp"`, `"crl_then_ocsp"`      | Which revocation sources are used.                       | Required: omitting the `[trust.revocation]` section or the `mode` key is a validation error (no silent default). `"none"` — revocation is not checked (NOT for production), must be set explicitly. |
