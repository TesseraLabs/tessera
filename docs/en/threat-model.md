...
| 11.1.3 | Web-cabinet static assets (SPA + WASM) | artifact substitution during delivery                                                  | the cabinet is embedded in the signed `issuer` binary and served by the agent same-origin by default — integrity via the binary signature, no CDN middleman; for external serving (`--cabinet-dir`/static) the `SHA256SUMS` manifest remains; keys are unreachable even from a substituted SPA (signing sits behind the agent + token). Residual risk — see 11.3 |
