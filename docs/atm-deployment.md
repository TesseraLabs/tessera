...
Этот раздел показывает, как Tessera разворачивается на парке банкоматов и
**кто из инженеров что сможет делать на устройстве**. Адресован инженерам
сопровождения и специалистам по безопасности банка: после прочтения видно
...
Банкомат — устройство без управляющей сети (zero-egress): в момент входа
...
- **`oper`** — оператор: повседневное обслуживание (замена кассет, просмотр
...
Что каждая роль разрешает на банкомате. «✓» — разрешено правами роли, «—» —
...
- **привязка к устройству** (host-binding) — сертификат выписан на конкретный
  банкомат; украденная флешка бесполезна на соседнем устройстве парка;
...
Для банкоматов рекомендуется режим **cert-only** — сертификат единственный
...
Доступ инженера к служебным операциям прикладного ПО банкомата задаётся
...
- **По команде** (нужна связь с устройством): список отзыва (CRL) и
  завершение живых сессий, карантин устройства. Доходит до банкомата при
  очередном сеансе связи.
- **По сроку** (работает всегда, без сети): сертификат протухает по TTL —
  часы или смена. Это backstop: даже на полностью изолированном банкомате
