...
`[trust.revocation].mode` ∈ {`none`, `crl`, `ocsp`, `crl_then_ocsp`}; каждый режим ДОЛЖЕН (MUST) иметь полноценную runtime-семантику (сведение к булеву `crl_strict` упразднено):
