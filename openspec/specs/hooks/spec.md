...
- ⚠ KNOWN GAP (docs): `post_auth_failure` из configuration.md:177 не существует.

### Requirement: Схема [[hooks]]

Каждая запись `[[hooks]]` ДОЛЖНА (MUST) иметь: `stage` (enum), `command` (Vec<String>, `command[0]` absolute), `timeout_seconds` (10, 1..=120; ⚠ док: 1..=60), `on_failure` (warn|ignore; иное/опечатка тихо → Abort; дефолт: pre_auth→Abort, прочие→Warn), `run_as` ("user"→PAM-user, иначе root), `env` (map значений-шаблонов).
...
- ⚠ KNOWN GAP: плейсхолдеры `${...}` работают ТОЛЬКО в `env`-значениях; `command` передаётся буквально (configuration.md:184–191 показывает argv-подстановку — неверно) (fork_exec.rs:104–110).
...
- ⚠ KNOWN GAP (testing): hook-security тесты (no_new_privs, uid-drop, fd-leak) — `#[ignore]` в CI из-за RLIMIT_NPROC=64 на shared-UID раннерах; верифицированы только вручную.
