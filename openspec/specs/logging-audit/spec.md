...
Поле serial ДОЛЖНО (MUST) оставаться пустым, пока серийник не приходит из источника, покрытого
...
разборе инцидента, поэтому пустое поле ДОЛЖНО (MUST) предпочитаться догадке.
