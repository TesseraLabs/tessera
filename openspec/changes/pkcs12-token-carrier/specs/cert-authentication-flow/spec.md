...
Идентификатор носителя ДОЛЖЕН (MUST) браться из того устройства, которое фактически несёт конверт: при источнике «раздел USB-носителя» — serial USB-устройства (flow.rs:887), при источнике «токен» — `CK_TOKEN_INFO.serialNumber` (как в режиме B, flow.rs:1253). Пустой serial токена ДОЛЖЕН (MUST) давать `TokenSerialMissing` и прерывать аутентификацию: идентификатор носителя входит в учёт сессии, и сессия без него неотличима от сессии любого другого носителя.

Извлечение токена НЕ отслеживается: демон наблюдает подсистему блочных устройств, а токен смарт-карты блочным устройством не является, и источника событий присутствия токена в системе нет. Режим мониторинга `strict` при источнике «токен» ДОЛЖЕН (MUST) отвергаться на входе — он обещает непрерывное присутствие носителя, которое для этого носителя необеспечимо; молча принятый strict создал бы уверенность в защите, которой нет.
...
#### Scenario: идентификатор носителя при источнике «токен»
- **WHEN** конверт прочитан с токена и аутентификация успешна
- **THEN** `AuthContext.usb_serial` содержит serial токена, и демон читает его как идентификатор токена, а не блочного устройства

#### Scenario: строгий мониторинг при источнике «токен»
- **WHEN** источником конверта задан объект данных на токене, а режим мониторинга — `strict`
- **THEN** аутентификация прерывается: непрерывное присутствие этого носителя не наблюдаемо, и обещать его нельзя
