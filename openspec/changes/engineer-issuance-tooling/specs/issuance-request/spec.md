...
### Requirement: Активная проба неизвлекаемости

Обратное чтение атрибутов НЕ ДОСТАТОЧНО (MUST NOT be treated as sufficient) для
вывода о неизвлекаемости ключа: провайдер, который хранит `CKA_EXTRACTABLE =
FALSE` и не обеспечивает его соблюдение, вернёт при чтении честное `FALSE` и
отдаст закрытый ключ по `CKA_VALUE` или `C_WrapKey`.

После генерации инструмент ДОЛЖЕН (MUST) выполнить активную пробу: попытаться
получить значение закрытого ключа. Проба, завершившаяся **успехом**, ДОЛЖНА
(MUST) приводить к отказу — ключ считается непригодным.

Инструмент ДОЛЖЕН (MUST) прочитать `CKA_NEVER_EXTRACTABLE` и
`CKA_ALWAYS_SENSITIVE` и отличать «никогда не был извлекаем» от «сейчас помечен
неизвлекаемым»: ключ, помеченный неизвлекаемым после того, как побывал
извлекаемым, ДОЛЖЕН (MUST) отвергаться.

Атрибут, которого устройство не вернуло вовсе, НЕ ДОЛЖЕН (MUST NOT)
истолковываться как удовлетворяющий требованию: отсутствие значения — отказ.

Проба, которую устройство не позволяет выполнить (нет механизма упаковки для
`C_WrapKey`), ДОЛЖНА (MUST) отражаться в отчёте как **невыполненная**, а не как
пройденная.

#### Scenario: Провайдер вернул честное FALSE и отдал ключ
- **WHEN** после генерации значение закрытого ключа читается успешно
- **THEN** операция завершается ошибкой; запрос на сертификат не собирается

#### Scenario: Атрибут не возвращён устройством
- **WHEN** устройство не вернуло `CKA_EXTRACTABLE`
- **THEN** операция завершается ошибкой; отсутствие значения не считается `FALSE`

#### Scenario: Ключ помечен неизвлекаемым, но был извлекаемым
- **WHEN** `CKA_EXTRACTABLE = FALSE`, но `CKA_NEVER_EXTRACTABLE = FALSE`
- **THEN** операция завершается ошибкой с указанием обоих значений

### Requirement: Поданный открытый ключ принадлежит ключу подписи

Режим с отдельно поданным открытым ключом ДОЛЖЕН (MUST) до сборки запроса
доказать, что поданный `SubjectPublicKeyInfo` принадлежит той же паре, которой
запрос будет подписан. Доказательство ДОЛЖНО (MUST) быть проверкой подписи под
поданным открытым ключом, а не сравнением метаданных.

#### Scenario: Подан чужой открытый ключ
- **WHEN** `--spki` указывает на ключ другой пары
- **THEN** сборка отвергается до подписи запроса, с указанием расхождения

### Requirement: Совместимость имён команд

Имя `csr` ДОЛЖНО (MUST) продолжать работать как скрытый алиас операции запроса
в течение одного релиза и ДОЛЖНО (MUST) печатать предупреждение об устаревании.

#### Scenario: Вызов по устаревшему имени
- **WHEN** инструмент вызван как `issuer csr`
- **THEN** операция выполняется, а на stderr печатается предупреждение с новым именем
