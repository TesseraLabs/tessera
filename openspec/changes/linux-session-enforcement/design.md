...
### 2. Supplementary-группы — replace набором роли (РИСК: app-dependent)

Целевое поведение: `setgroups` устанавливает **ровно** группы из роли, заменяя
набор. Общая техническая учётка сама по себе бесправна — весь доступ только от
роли (чистый least-privilege, суть продукта). Базовые группы (tty и пр.), если
нужны, добавляются в роль явно.

**РИСК — не гарантировано, проверить в реализации.** `setgroups` в `setcred`
влияет на login-процесс, дочерний шелл наследует. Но многие login-приложения
сами вызывают `initgroups()` (из `/etc/group`) рядом с `setuid`. Если приложение
делает свой `initgroups` **после** `pam_setcred` — оно перезатрёт наш набор
группами учётки (у общей техучётки их почти нет → тихий no-op / потеря replace).
Порядок `initgroups` vs `setcred` различается: sshd обычно `initgroups` до
`setcred` (наш набор ложится поверх, ок); shadow `login`, fly-dm и прочие
greeters — проверять каждый. Это тот же механизм, что у `pam_group.so` —
жизнеспособен в типовых конфигах, но эффект зависит от приложения.

**Блокирующий прототип** (до основной реализации): проверить фактический
порядок и итоговый набор групп в сессии на целевых приложениях Astra SE
(fly-dm, `login`, `sshd`, `sudo`) и Debian. Критерий — группы роли реально
присутствуют в дочернем процессе сессии.

**Fallback при перетирании — NSS-путь.** Если прототип показывает, что
приложение перетирает набор, давать группы роли через NSS-модуль
(`nss_tessera`): `initgroups()`/`getgrouplist()` приложения возвращает группы
активной роли. Работает с любым приложением (штатный путь получения групп),
не зависит от порядка. Цена — отдельный компонент + «активная роль» должна быть
доступна NSS в момент `initgroups` (состояние/сокет). Выбор setgroups vs NSS
фиксируется по итогу прототипа.
