...
Терминальный парк (zero-egress) живёт на `mode="none"`+короткий TTL и offline CRL — для него ничего
...
- Терминальный продакшн (`mode="none"`) не затрагивается; gost-engine/PKCS#11 пути не затрагиваются.
