...
- [ ] 20. Зарезервированные имена ролей: пространство имён ролей совпало с системными УЗ, `root`/`daemon`/`nobody` проходят синтаксис. Решить — запрет в `RoleId` или ответственность Census, зафиксировать в спеке. Отдельным изменением
- [ ] 21. `docs/{ru,en}/install.md` заводит ролевую УЗ с логин-шеллом, не закрывая `ssh`/`su`/`sudo -u`, при том что документация обещает «вход закрыт всеми путями кроме Tessera». Цена обхода выросла с личной УЗ до общей ролевой
- [ ] 22. Заявление о совместимости (старый лист с отозванным расширением проходит) не покрыто тестом; фикстуры суффиксной модели — единственная его опора
- [ ] 23. Синхронизация `openspec/specs/README.md` при архивации (строки 15, 17, 60, 69)
