...
Недоступный для записи stderr НЕ ДОЛЖЕН (MUST NOT) прерывать операцию: выпуск, потерянный
из-за невозможности напечатать предупреждение, вреднее ненапечатанного предупреждения.
...
#### Scenario: Источник назван для другого бэкенда
- **WHEN** задан флаг источника, относящийся не к выбранному бэкенду (например `--key-passphrase-file` при `--backend pkcs11`)
- **THEN** запуск отвергается с указанием флага и выбранного бэкенда: названный оператором источник не должен молча игнорироваться в пользу другого
