...
`qr-login-core` даёт вход по QR в текстовом виде (режим 1) — работает на TTY/SSH, но не на
графическом входе Astra и без графики. На production-терминалах Astra (fly-dm, тема `fly-modern`)
нужен **графический QR на экране логина**, не ломая родную тему. `fly-modern` (UiGreeter) плагины
не грузит, а PAM info-сообщения **вообще не доходят до экрана** (живой прогон 2026-08-18,
`tessera-ws/specs/2026-07-03-qr-login-display-design.md` §6.1; прежняя формулировка «выбрасывает
модальным `QMessageBox`» была выводом из чтения `themewidget.cpp` и опровергнута). Вывод от этого
только крепче: показать картинку штатным каналом темы нельзя тем более.

Текст промпта тема, напротив, рисует целиком — но это канал для ТЕКСТА: изображение им не
передать, а UTF-8-QR рвёт пропорциональный шрифт панели. Поэтому текстовая диктовка телефонного
канала оверлея НЕ требует (она ушла в текст промпта — `greeter-prompt-channel`), а графический
QR требует по-прежнему. Решение (спайк-верифицировано на живой Astra 1.8.4): внешний X-оверлей
поверх greeter'а.
