...
get_user_mnkc, apply_session, get/set_file_label включая fd-вариант) плюс `init(config)` /
`teardown`.
...
нести идентификатор алгоритма (расширяемость Ed25519/ГОСТ).
...
быть обёрнута catch_unwind: паника плагина → ошибка PAM-вызова (fail-closed) +
audit `plugin_panic`, не UB и не тихая деградация. Ошибка `init` → плагин отвергнут,
StubBackend.
