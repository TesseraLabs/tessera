# Миграция OpenSpec → spec9

## Результат

Все 32 capability и 202 заголовка `### Requirement:` из `openspec/specs/`
имеют единственного владельца среди норм spec9. Проверка выполняется командой:

```bash
node spec9/tools/spec.mjs coverage --missing
```
...
начале раздела; дальнейшие изменения вносятся только в spec9.
