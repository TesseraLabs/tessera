---
id: revocation-semantics
kind: операция
context: auth
name: "Семантика проверки отзыва"
aliases:
  - "revocation"
relations:
  references:
...
anchors:
  code:
    - "crates/tessera_core/src/trust/openssl_verifier.rs"
  test:
    - "tests/e2e/cases/40-revocation.yaml"
requirements:
  REVS-001:
    kind: инвариант
    subjects:
      - auth.revocation-semantics
    origins:
      - "revocation::Режимы и их фактическая семантика"
    evidence:
      test:
        - "tests/e2e/cases/40-revocation.yaml"
  REVS-002:
  ...
  REVS-003:
    ...
    subjects:
      - auth.revocation-semantics
    origins:
      - "revocation::Доверие к самой CRL"
    evidence:
      test:
        - "tests/e2e/cases/40-revocation.yaml"
  REVS-004:
    ...
    subjects:
      - auth.revocation-semantics
    origins:
      - "revocation::Компенсация отсутствия отзыва"
    evidence:
      code:
        - "crates/tessera_core/src/trust/openssl_verifier.rs"
  REVS-005:
  ...
  REVS-006:
    ...
    evidence:
      code:
        - "crates/tessera_core/src/trust/openssl_verifier.rs"
---
# Семантика проверки отзыва

Эта страница заменяет процедурную capability-спеку OpenSpec «revocation»: она
...
### REVS-001 — Режимы и их фактическая семантика

[[auth.revocation-semantics|Семантика проверки отзыва]] MUST соблюдать правило «Режимы и их фактическая семантика» вместе со всеми условиями и отказами, зафиксированными ниже.

Мигрировано из openspec/specs/revocation/spec.md, requirement «Режимы и их фактическая семантика».

`[trust.revocation].mode` ∈ {`none`, `crl`, `ocsp`, `crl_then_ocsp`} и должен быть задан явно: c 2026-07 пропуск секции `[trust.revocation]` или ключа `mode` — ошибка валидации конфига (молчаливого дефолта `none` больше нет; отказ от проверки отзыва требует явного `mode = "none"`). Каждый режим должен иметь полноценную runtime-семантику (сведение к булеву `crl_strict` упразднено):
...
Anchor-серты через OCSP НЕ проверяются (доверие к anchor = trust store). В OCSP-режимах неопределимость статуса отзыва должна приводить к отказу аутентификации (fail-closed, `PAM_AUTH_ERR`) — деградации «WARN и пропустить» в OCSP-режимах нет.
...
### REVS-002 — CRL-проверка

[[auth.revocation-semantics|Семантика проверки отзыва]] MUST соблюдать правило «CRL-проверка» вместе со всеми условиями и отказами, зафиксированными ниже.

Мигрировано из openspec/specs/revocation/spec.md, requirement «CRL-проверка».

`check_revocation` должна для каждой CRL и каждого серта цепочки (включая anchor): если serial (lowercase hex) в `crl.revoked` → `TrustError::Revoked` (store.rs). Пустой store → Ok («нет CRL = нет проверки»).

CRL должна считаться stale, если выполняется хотя бы одно из условий:
...
### REVS-003 — Доверие к самой CRL

[[auth.revocation-semantics|Семантика проверки отзыва]] MUST соблюдать правило «Доверие к самой CRL» вместе со всеми условиями и отказами, зафиксированными ниже.

Мигрировано из openspec/specs/revocation/spec.md, requirement «Доверие к самой CRL».

`check_revocation` должна применять CRL только к сертам, чей issuer DN байт-в-байт (DER) совпадает с issuer DN CRL (RFC 5280 §6.3.3). При несовпадении или при ошибке DER-кодирования issuer-имени серта (scope недоказуем) данная CRL к данному серту НЕ применяется.

Перед применением CRL к серту её подпись должна быть верифицирована публичным ключом issuer-сертификата из проверяемой цепочки (`verify_signature_with_issuer`; issuer ищется по subject DN == issuer DN CRL — цепочка leaf-first и полна вплоть до self-signed anchor, для GOST-issuer'ов предварительно поднимается gost-engine). Невалидная подпись → `TrustError::CrlSignatureInvalid` — fail-closed, тот же класс отказа, что и `Revoked`, и НЕ зависит от `crl_strict`. Подпись верифицируется не более одного раза на CRL за вызов.
...
### REVS-004 — Компенсация отсутствия отзыва

[[auth.revocation-semantics|Семантика проверки отзыва]] MUST соблюдать правило «Компенсация отсутствия отзыва» вместе со всеми условиями и отказами, зафиксированными ниже.

Мигрировано из openspec/specs/revocation/spec.md, requirement «Компенсация отсутствия отзыва».

При `mode="none"` (текущий продакшн-конфиг терминалов) компенсация должна обеспечиваться коротким TTL leaf-сертификатов (рекомендация: ≤30 дней; для привилегированных операций ≤7 дней / 24ч) — зафиксировано как deployment-политика (May 2026).
...
### REVS-005 — OCSP-клиент

[[auth.revocation-semantics|Семантика проверки отзыва]] MUST соблюдать правило «OCSP-клиент» вместе со всеми условиями и отказами, зафиксированными ниже.

Мигрировано из openspec/specs/revocation/spec.md, requirement «OCSP-клиент».

OCSP-клиент должен: строить OCSPRequest через OpenSSL-примитивы (CertID по issuer'у, nonce RFC 8954); выполнять один HTTP POST (`application/ocsp-request`) на `ocsp_responder_url` без redirect'ов и keep-alive; укладываться в общий deadline `ocsp_timeout_seconds` (connect + write + read); ограничивать размер ответа (1 MiB). Ответ должен приниматься только если: `OCSPResponseStatus = successful`, подпись responder'а верифицируется цепочкой к `[trust]` anchors (delegated responder — только с EKU id-kp-OCSPSigning от issuer'а проверяемого серта), окно thisUpdate/nextUpdate валидно с учётом `clock_skew_seconds`, и nonce (если присутствует в ответе) совпадает с запросом. Статус `unknown` должен трактоваться как отказ. Для GOST-подписанных ответов перед верификацией должен загружаться gost-engine (`ensure_loaded_if_any_gost`, fail-closed при отсутствии).
...
[[auth.revocation-semantics|Семантика проверки отзыва]] MUST соблюдать правило «OCSP-кэш» вместе со всеми условиями и отказами, зафиксированными ниже.

Мигрировано из openspec/specs/revocation/spec.md, requirement «OCSP-кэш».

Кэш должен хранить DER-ответы в `/var/cache/tessera/ocsp/` (0640 root:root; имя файла — hex(sha256(CertID))). Запись валидна до min(`nextUpdate` ответа, mtime + `ocsp_cache_ttl_seconds`); `ocsp_cache_ttl_seconds = 0` — кэш выключен. Кэшироваться должны только определённые статусы (`good`, `revoked`); `unknown` не должен кэшироваться. Кэшированный ответ перед использованием должен проходить повторную верификацию (подпись, окно валидности) — файл кэша не является доверенным входом; повреждённый/неверифицируемый файл трактуется как cache miss (+WARN), не как отказ.
