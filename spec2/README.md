...
Человеческое представление не реализуется отдельным UI внутри spec2. Repo-local
skill [tessera-spec-review](../.agents/skills/tessera-spec-review/SKILL.md)
передаёт Markdown semantic review существующему annotator (для Plannotator —
через его готовый skill), раскрывает только выбранные нормы/ADR/процессы и после
сессии удаляет временный документ. Комментарии остаются в annotator/MR, а не
становятся параллельным артефактом спецификации.
