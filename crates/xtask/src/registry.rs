...
pub enum RegistryError {
    ...
    Invalid(String),
    /// Кейс зовёт хелпер, которого в `tests/e2e/helpers/` нет.
    #[error(
        "{path}: кейс {case}: вызывается helpers/{helper}, которого нет — \
         переименованный или ненаписанный хелпер валит кейс сбоем стенда, \
         а выглядит это как отказ продукта"
    )]
    MissingHelper {
        ...
        /// Идентификатор кейса.
        case: String,
        /// Имя хелпера, как оно записано в шаге.
        helper: String,
    },
    /// Значение `requirement` не является канонической ссылкой на спеку.
    #[error(
        "{path}: кейс {case}: ссылка {requirement} не ведёт на спеку; \
         ожидается стабильный ID вида spec9:<context>.<ПРЕФИКС>-<НОМЕР>"
    )]
...
impl Registry {
    ...
    ///
    /// См. [`Registry::load`].
    pub fn load_from(dirs: &[PathBuf], _repo_root: &Path) -> Result<Self, RegistryError> {
        let mut registry = Self::default();
        for dir in dirs {
            registry.load_dir(dir)?;
        }
        registry.validate()?;
        registry.check_helpers()?;
        registry.check_requirements()?;
        Ok(registry)
    }
    ...
    /// Проверяет, что каждый хелпер, который зовут кейсы, лежит на диске.
    ///
    /// Переименованный хелпер не ломает ни разбор реестра, ни сборку: кейс
    /// падает на стенде «команда не найдена», и отличить это от отказа продукта
    /// можно только чтением прогона. Проверка дешёвая, а класс дефекта прожил в
    /// ветке до ревью.
    ///
    /// Проверяется ТОЛЬКО вызов с префиксом `helpers/`. Инструменты стенда,
    /// которые кейс зовёт по имени из `PATH` — `pam-drive`, собранный раннером
    /// из `helpers/pam-drive.c`, — отсюда неотличимы от `grep` и любой другой
    /// системной команды, и требовать для них файла в `helpers/` значило бы
    /// падать на каждой команде вообще.
    ///
    /// Существование ВЕТВИ хелпера здесь не проверяется тоже: подкоманды
    /// разбирает сам хелпер, и реестру пришлось бы читать shell.
    fn check_helpers(&self) -> Result<(), RegistryError> {
        for suite in &self.suites {
            let Some(helpers) = suite.root.parent().map(|dir| dir.join("helpers")) else {
                continue;
            };
            for case in &suite.cases {
                for step in case.steps.iter().chain(case.teardown.iter()) {
                    let Step::Run(run) = step else { continue };
                    for helper in helper_names(&run.run) {
                        if helpers.join(&helper).is_file() {
                            continue;
                        }
                        return Err(RegistryError::MissingHelper {
    ...
    /// Проверяет форму каждой ссылки `requirement`.
    ///
    /// Публичный реестр хранит только стабильный ID нормы. Существование нормы
    /// и её evidence проверяет Spec9 в приватном workspace, где доступна сама
    /// спецификация; публичный репозиторий не должен зависеть от её пути.
    fn check_requirements(&self) -> Result<(), RegistryError> {
        for suite in &self.suites {
            for case in &suite.cases {
                spec_name(&case.requirement).ok_or_else(|| {
...
/// Достаёт из команды имена хелперов, которые она зовёт.
///
/// Команда исполняется через `sh -lc`, поэтому хелпер может стоять не только
/// первым словом: кейс вправе звать его из подстановки или после `&&`.
fn helper_names(command: &str) -> Vec<String> {
    const PREFIX: &str = "helpers/";
    command
...
/// Идентификатор нормы из канонической публичной ссылки `spec9:<context>.<ID>`.
///
/// Контекст состоит из строчных ASCII-букв, цифр, `_` и `-`; префикс — из
/// заглавных ASCII-букв и цифр; номер — только из цифр. Семантическое
/// существование ID проверяет Spec9 в приватном workspace.
#[must_use]
pub fn spec_name(requirement: &str) -> Option<&str> {
    let id = requirement.trim().strip_prefix("spec9:")?;
    let (context, local_id) = id.split_once('.')?;
    if context.is_empty()
        || !context
            .chars()
...
"
    ...
    /// Готовит дерево с кейсом, зовущим указанные команды, и каталогом хелперов.
    fn tree_with_helper_calls(steps: &str, helpers: &[&str]) -> (tempfile::TempDir, PathBuf) {
        let root = tempfile::tempdir().unwrap();
        let cases = root.path().join("tests/e2e/cases");
        std::fs::create_dir_all(&cases).unwrap();
        let helpers_dir = root.path().join("tests/e2e/helpers");
        std::fs::create_dir_all(&helpers_dir).unwrap();
        for helper in helpers {
            std::fs::write(helpers_dir.join(helper), "#!/bin/sh\n").unwrap();
        }
        std::fs::write(
            cases.join("auth.yaml"),
            format!(
...
cases:
  - id: A-1
    title: t
    requirement: spec9:runtime.PLAN-001
    steps:
{steps}
"
            ),
    ...
    #[test]
    fn a_case_calling_a_helper_that_exists_loads() {
        let (root, cases) =
            tree_with_helper_calls("      - run: helpers/codes.sh prepare\n", &["codes.sh"]);
        Registry::load_from(std::slice::from_ref(&cases), root.path()).unwrap();
    }

    #[test]
    fn a_case_calling_a_renamed_helper_is_refused() {
        let (root, cases) = tree_with_helper_calls(
            "      - run: helpers/codes-phone.sh prepare\n",
            &["codes.sh"],
        );
        let error = Registry::load_from(std::slice::from_ref(&cases), root.path()).unwrap_err();
        assert!(
            matches!(&error, RegistryError::MissingHelper { helper, case, .. }
                if helper == "codes-phone.sh" && case == "A-1"),
            "{error}"
        );
    }

    #[test]
    fn a_helper_missing_from_a_teardown_is_refused() {
        let root = tempfile::tempdir().unwrap();
        let cases = root.path().join("tests/e2e/cases");
        std::fs::create_dir_all(&cases).unwrap();
        std::fs::create_dir_all(root.path().join("tests/e2e/helpers")).unwrap();
        std::fs::write(
            cases.join("auth.yaml"),
            r"
suite: s
cases:
  - id: A-1
    ...
    steps:
      - run: /bin/true
    teardown:
      - run: helpers/gone.sh cleanup
",
        )
        .unwrap();
        let error = Registry::load_from(std::slice::from_ref(&cases), root.path()).unwrap_err();
        assert!(
            matches!(&error, RegistryError::MissingHelper { helper, .. } if helper == "gone.sh"),
            "{error}"
        );
    }

    /// Инструмент, который кейс зовёт по имени из `PATH`, проверке не подлежит.
    ///
    /// `pam-drive` раннер собирает из `helpers/pam-drive.c` и кладёт в `PATH`;
    /// в вызове кейса он выглядит ровно как `grep` или `install`, и требовать
    /// для него файла в `helpers/` значило бы падать на любой команде. Предел
    /// назван тестом, а не комментарием, чтобы попытка «заодно проверить и
    /// такие» ломала его и заставляла подумать.
    #[test]
    fn a_tool_called_by_bare_name_is_not_taken_for_a_helper() {
        assert!(helper_names("pam-drive certauth user authenticate").is_empty());
        assert_eq!(
            helper_names("grep -c op /var/lib/tessera/audit.ndjson"),
            Vec::<String>::new()
        );
    }

    /// A `helpers/` segment inside another path does not name a repository helper.
    #[test]
    fn a_helpers_segment_inside_another_path_is_not_taken_for_a_helper() {
        let (root, cases) =
            tree_with_helper_calls("      - run: /tmp/helpers/pam-drive svc u auth\n", &[]);

        Registry::load_from(std::slice::from_ref(&cases), root.path()).unwrap();
    }

    #[test]
    fn a_helper_called_from_a_compound_command_is_seen() {
        assert_eq!(
            helper_names("cd /tmp && helpers/one.sh a; helpers/two.sh b"),
            vec!["one.sh".to_owned(), "two.sh".to_owned()]
        );
    }

    #[test]
    fn a_private_spec9_norm_id_does_not_require_the_private_repository() {
        let (root, cases) = tree_with_requirement("spec9:auth.REVS-001");
        let registry = Registry::load_from(std::slice::from_ref(&cases), root.path()).unwrap();
        assert_eq!(registry.cases().count(), 1);
        assert_eq!(spec_name("spec9:auth.REVS-001"), Some("auth.REVS-001"));
    }

    #[test]
    fn only_canonical_spec9_ids_are_accepted() {
        for requirement in [
            "spec9:auth.REVS-001",
            "spec9:runtime.PLAN-002",
            " spec9:codes.R2D2-42 ",
        ] {
            assert!(spec_name(requirement).is_some(), "{requirement}");
        }
        for requirement in [
            "Cargo.toml",
            "r",
            "spec9/operations/auth/revocation-semantics.md#REVS-001",
            "../spec9/README.md",
            "spec9:revs-001",
            "spec9:REVS-001",
            "spec9:Auth.REVS-001",
    ...
    #[test]
    fn a_non_spec9_requirement_is_rejected_before_the_run() {
        let (root, cases) = tree_with_requirement("r");
        let err = Registry::load_from(std::slice::from_ref(&cases), root.path())
            .expect_err("публичный реестр принимает только стабильные Spec9 ID");
        assert!(
            matches!(err, RegistryError::MalformedRequirement { .. }),
            "{err}"
        );
        let text = err.to_string();
        assert!(text.contains("spec9:<context>.<ПРЕФИКС>-<НОМЕР>"), "{text}");
    }
