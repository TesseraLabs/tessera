...
fn loads_rsa_p12() {
    ...
    let m = LoadedKeyMaterial::from_p12(RSA, &pin, None).unwrap();
...
fn loads_ecdsa_p12() {
    let pin = SecretString::from("correct-pin".to_string());
    let m = LoadedKeyMaterial::from_p12(ECDSA, &pin, None).unwrap();
...
fn wrong_pin_returns_wrong_pin() {
    ...
    let err = LoadedKeyMaterial::from_p12(RSA, &pin, None).unwrap_err();
...
fn corrupt_p12_returns_corrupt() {
    let pin = SecretString::from("correct-pin".to_string());
    let err = LoadedKeyMaterial::from_p12(b"not-a-p12-byte-stream", &pin, None).unwrap_err();
...
fn empty_input_returns_corrupt() {
    let pin = SecretString::from("correct-pin".to_string());
    let err = LoadedKeyMaterial::from_p12(b"", &pin, None).unwrap_err();
    assert!(matches!(err, Pkcs12Error::Corrupt(_)), "got {err:?}");
}
