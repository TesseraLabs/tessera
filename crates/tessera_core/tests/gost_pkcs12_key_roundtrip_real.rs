...
use crate::common::{fixture_path, gost_fixtures_dir, gost_fixtures_present};
...
fn round_trip(p12_name: &str) {
    ...
    let material = LoadedKeyMaterial::from_p12(&bytes, &pin, None)
...
fn gost_containers_open_without_a_preloaded_engine() {
    ...
    // all, and only report a failure when it is.
    let bytes = std::fs::read(fixture_path("gost_ee_256.p12")).expect("read fixture");
    let pin = SecretString::from(FIXTURE_PIN.to_owned());
    if let Err(e) = LoadedKeyMaterial::from_p12(&bytes, &pin, None) {
        assert!(
            tessera_core::gost::engine::is_available_after_attempt(None),
            "gost_ee_256.p12 failed to open and gost-engine is not loadable on this host \
             either — nothing to test here, but the fixtures suggest it should be: {e}",
        );
        panic!("gost_ee_256.p12 must open on a host where gost-engine is loadable: {e}");
    }

    round_trip("gost_ee_256.p12");
    round_trip("gost_ee_512.p12");
}
