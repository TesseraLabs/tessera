...
- **REQ/ACC/PLC (14), `62-issuer-request`** — suite заведён 2026-08-11 вместе со
  спекой, кода ещё нет: команд `issuer request` и `issuer accept` не существует.
  Кейсы красные по построению, ожидания взяты из спеки, а не из прогона —
  так и задумано правилом «кейс раньше кода». Внутри suite три группы, и
  красные они по разным причинам:
  - REQ-0xx, ACC-*: нет реализации команд;
  - REQ-1xx, PLC-101: требуют `hardware-token`, которого не объявляет ни один
    профиль (USB-проброс в виртуальную машину не настроен) — дадут `SKIP`, как
    и `70-token-carrier`. Живой Рутокен есть только на рабочем Mac, стенда под
    него в контуре нет;
  - REQ-2xx: требуют подставной PKCS#11-модуль `crates/tessera_pkcs11_liar`,
    который пишется следующей задачей волны; до него `{{liar_module}}`
    указывает в пустоту;
  - ACC-*, PLC-101 дополнительно ждут хелпера `issuer-request-fixture.sh`.
